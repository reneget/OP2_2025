# Client — CLI для сортировки расчёсткой
...
docker attach sorting-client
```

После этого вы увидите консоль клиента прямо в терминале.

### Настройка URL сервера (для локального запуска)
...
После выполнения сортировки отображается результат (пример):
